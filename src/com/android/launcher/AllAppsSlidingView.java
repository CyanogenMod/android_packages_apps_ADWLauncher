...
public class AllAppsSlidingView extends AdapterView<ApplicationsAdapter>
	...
	Drawer {// implements DragScroller{
    ...
    private int SWIPE_MIN_DISTANCE = (int) (ABS_SWIPE_MIN_DISTANCE
	    * dm.densityDpi / 160.0f);
    private int SWIPE_MAX_OFF_PATH = (int) (ABS_SWIPE_MAX_OFF_PATH
	    * dm.densityDpi / 160.0f);
    private int SWIPE_THRESHOLD_VELOCITY = (int) (ABS_SWIPE_THRESHOLD_VELOCITY
	    * dm.densityDpi / 160.0f);
	...
	paginatorSpace = a.getDimensionPixelSize(
		R.styleable.AllAppsSlidingView_pager_height, paginatorSpace);
	a.recycle();
	initWorkspace();
    }
	...
	AllAppsSlidingView.LayoutParams holderParams = new AllAppsSlidingView.LayoutParams(
		ViewGroup.LayoutParams.MATCH_PARENT,
		ViewGroup.LayoutParams.MATCH_PARENT);
	holder.layout(pageSpacing, paginatorSpace, pageSpacing + mPageWidth,
		getMeasuredHeight());
	...
	selectorRect.set(sel.getLeft(), sel.getTop() + paginatorSpace,
		sel.getRight(), sel.getBottom() + paginatorSpace);
    ...
    /*
     * private void RecycleOuterViews(int screen){ final int
     * startPos=(screen*mNumColumns*mNumRows);//-mFirstPosition; final int
     * endPos=startPos+(mNumColumns*mNumRows)-1; final int
     * childCount=getChildCount(); int recycledCount=0; for(int
     * i=childCount-1;i>=0;i--){ if(i<startPos || i>endPos){ View
     * child=getChildAt(i); mRecycler.addScrapView(child);
     * detachViewFromParent(child); recycledCount++; } }
     * mLayoutMode=LAYOUT_NORMAL; }
     */// Seems that this method is not be used
    /**
	...
	mLauncher.showActions(app, v, new PopupWindow.OnDismissListener() {
		...
		if (!mLauncher.isDockBarOpen()
			|| AlmostNexusSettingsHelper
				.getUICloseAppsDockbar(mLauncher)) {
	...
	if (animate) {
		...
		((ApplicationsAdapter) adapter)
			.setChildDrawingCacheEnabled(true);
	...
	if (mFadeType != FADE_OFF) {
		...
		if (time > 0) {
			...
			private void fadeIn() {
				...
				mFadeType = FADE_IN;
				mFadeEnd = System.currentTimeMillis()
					+ (150 + time);
			    }
			}
		    };
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
		...
		if (e1.getY() - e2.getY() > SWIPE_MIN_DISTANCE
		    ...
		    // Toast.makeText(getContext(), "Up Swipe",
		    // Toast.LENGTH_SHORT).show();
		} else if (e2.getY() - e1.getY() > SWIPE_MIN_DISTANCE
			&& Math.abs(velocityY) > SWIPE_THRESHOLD_VELOCITY) {
		    mLauncher.navigateCatalogs(Launcher.ACTION_CATALOG_PREV);
		    // Toast.makeText(getContext(), "Down Swap",
		    // Toast.LENGTH_SHORT).show();
		}
	    } catch (Exception e) {
		// nothing
